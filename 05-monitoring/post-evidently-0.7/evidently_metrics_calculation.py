...
def calculate_metrics_postgresql(i):
	...
	fare_amount_q50 = result['metrics'][3]['value']
	fare_amount_missing = result['metrics'][4]['value']['count']
	with psycopg.connect(CONNECTION_STRING_DB, autocommit=True) as conn:
		with conn.cursor() as curr:
			curr.execute(
				"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, fare_amount_q50, fare_amount_missing) values (%s, %s, %s, %s, %s, %s)",
				(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, fare_amount_q50, fare_amount_missing)
			)
